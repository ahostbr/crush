...
var homeIgnorePatterns = sync.OnceValue(func() []gitignore.Pattern {
	...
	for _, name := range []string{
		...
		filepath.Join(homeDir, ".config", "kuroryuu", "ignore"),
	} {
...
type directoryLister struct {
	// dirPatterns caches parsed patterns from .gitignore/.kuroryuuignore for each directory.
...
// .gitignore and .kuroryuuignore files. Results are cached.
func (dl *directoryLister) getDirPatterns(dir string) []gitignore.Pattern {
	return dl.dirPatterns.GetOrSet(dir, func() []gitignore.Pattern {
		...
		for _, ignoreFile := range []string{".gitignore", ".kuroryuuignore"} {
