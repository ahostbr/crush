...
func TestCrushIgnore(t *testing.T) {
	...
	// Create a .kuroryuuignore file that ignores .log files
	require.NoError(t, os.WriteFile(".kuroryuuignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	// Create .kuroryuuignore file
	crushignoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".kuroryuuignore"), []byte(crushignoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .kuroryuuignore: %v", err)
	}
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .kuroryuuignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .kuroryuuignore in subdir that ignores normal_nested
	subCrushignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".kuroryuuignore"), []byte(subCrushignore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .kuroryuuignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .kuroryuuignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .kuroryuuignore")
